...
class OrderController extends Controller
{
    public function __construct()
    {
        $this->middleware(function ($request, $next) {
            ...
            if (!$this->user || Helper::hasRight('enroll.view') == false) {
    ...
    public function index(){
        ...
        return view('backend.pages.order.index', compact('category','brands','users', 'products', 'companies'));
    }
    ...
    public function getList(Request $request){
        ...
        ->editColumn('payment_status', function ($row) {
            if ($row->payment_status == 0) {
                return '<span class="badge bg-danger w-50">Pending</span>';
            }elseif ($row->payment_status == 1) {
                return '<span class="badge bg-success w-50">Paid</span>';
            }
        })

        ->editColumn('payment_method', function ($row) {
            return '<span class="badge bg-primary w-50">'.$row->payment_method.'</span>';
        })

        ->editColumn('status', function ($row) {
            if ($row->status == 0) {
                return '<span class="badge bg-info w-80">Enrolled</span>';
            }elseif ($row->status == 1) {
                return '<span class="badge bg-warning w-80">Droped</span>';
            }elseif ($row->status == 2) {
                return '<span class="badge bg-success w-80">Completed</span>';
            }else{
        ...
        ->addColumn('action', function ($row) {
            ...
            if ($this->user->role === 4 || $this->user->role === 5) {
                ...
                $btn = '';
                if (Helper::hasRight('enroll.edit')) {
                    $btn = $btn . '<a href="" data-id="'.$row->id.'" class="status_change_btn btn btn-sm btn-info text-light"><i class="fa-solid fa-circle-check"></i></a>';
                }
                if (Helper::hasRight('enroll.edit')) {
                    $btn = $btn . '<a href="" data-id="'.$row->id.'" class="edit_btn btn btn-sm btn-primary mx-1"><i class="fa-solid fa-pencil"></i></a>';
                }
                if (Helper::hasRight('enroll.delete')) {
                    $btn = $btn . '<a class="delete_btn btn btn-sm btn-danger " data-id="'.$row->id.'" href=""><i class="fa fa-trash" aria-hidden="true"></i></a>';
                }
                return $btn;
            })
        ->rawColumns(['user_id','status', 'payment_method', 'payment_status', 'action'])->make(true);
    }
    ...
    public function row($number){
        ...
        $number++;
        $companies = Company::where('status', 1)->orderBy('name', 'asc')->get();
        return view('backend.pages.order.row', compact('products','number', 'companies'));
    }
    ...
    public function getProductByCompany(Request $request){
        $products = Product::select('id', 'name')->where('company_id', $request->company_id)->whereStatus(1)->get();
        ...
        if ($order->save()) {
            ...
            for ($i=0; $i < count($request->product); $i++) {
                if(!empty($request->product[$i])){
                    ...
                    $order_detail->company_id = $request->partner_id[$i];
    ...
    public function edit($order_id){
        ...
        $users = User::whereIn('role', [2])->where('status', 1)->get();
        $companies = Company::where('status', 1)->orderBy('name', 'asc')->get();
        return view('backend.pages.order.edit', compact('order','products','partners','billing', 'users', 'companies'));
    }
