...
class Helper
{
    ...
    public static function hasRight($right, $role_id = null)
    {
        ...
        if (!$right) {
            // If the right does not exist, return false
            return false;
        }

        // Check if the role has the specified permission
        if (RoleRight::where('role_id', $role)
    ...
    public static function getfrontensection($key)
    {
        $section = FrontendSection::where('key', $key)->first();
        if (!is_null($section)) {
    ...
    public static function getalumnisection($key)
    {
        $section = AlumniSection::where('key', $key)->first();
        if (!is_null($section)) {
            return $section->value;
        } else {
            return false;
