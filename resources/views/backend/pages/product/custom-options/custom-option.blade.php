<form id="customProductForm" action="{{ route('admin.course.update.custom.option', $product->id) }}" method="post"
            ...
            <div class="list-group b-0">
                @foreach($custom_options as $row)
                    <li class="list-group-item list-group-item-action d-flex justify-content-between">
                        ...
                        <div class="d-flex">
                            <a href="#" type="button" onclick="editCustomOption('{{$row->custom_field_id}}', '{{$row->sub_option}}', '{{$row->product_id}}'); return false;" class="edit_custom_option_btn"><i class="fa-solid fa-pen-to-square"></i></a>
                            <a href="#" type="button" onclick="deleteCustomOption('{{$row->custom_field_id}}', '{{$row->id}}', '{{$row->product_id}}'); return false;" class="delete_custom_option text-danger ms-1"><i class="fa-solid fa-trash"></i></a>
                        </div>
                    </li>
                @endforeach
            </div>
        ...
        <div class="col-lg-8">
            ...
            <div class="row select_custom_field_area">
                ...
                <div class="col-lg-2 d-flex align-items-center">
                    <button type="button" class="btn btn-primary generate_html_btn" data-product-id="{{ $product->id }}">Add</button>
                </div>
            </div>
            <div class="row">
                <div class="col-lg-12">
